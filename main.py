...
if st.session_state.is_running and total_found < gen_limit:
    ...
    # 1. حساب الطول المطلوب للجسم
    needed_body_len = max(0, u_len - len(u_prefix))
    
    if use_under:
        # إذا تم تفعيل الشرطة، نضمن وجودها إجبارياً
        if needed_body_len > 0:
            # نختار مكان عشوائي للشرطة (ليس في الخانة الأخيرة)
            # بما أن البداية قد تكون Prefix، نركز أن لا تكون في آخر خانة باليوزر الكلي
            body_list = random.choices(chars, k=needed_body_len)
            
            # تحديد المواقع المتاحة للشرطة (تجنب أول خانة إذا لم يوجد Prefix، وتجنب آخر خانة دائماً)
            full_user_temp = list(u_prefix + "".join(body_list))
            possible_indices = [i for i in range(1, len(full_user_temp)-1)]
            
            if possible_indices:
                idx = random.choice(possible_indices)
                full_user_temp[idx] = "_"
                user = "".join(full_user_temp)
            else:
                # إذا كان اليوزر قصير جداً لا يسمح بشرطة في المنتصف
                user = u_prefix + "".join(body_list)
        else:
            user = u_prefix
    else:
        # توليد عادي بدون شرطة
        user = u_prefix + "".join(random.choices(chars, k=needed_body_len))
