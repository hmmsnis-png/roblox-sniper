...
# إدارة البيانات لكل مستخدم بشكل منفصل (Session State)
...
with left_col:
    ...
    # زر حفظ اليوزرات المتاحة
    if st.session_state.data["valid"]:
        valid_text = "\n".join(st.session_state.data["valid"])
        st.download_button(
            label="📥 Save Valid (.txt)" if not is_ar else "📥 حفظ المتاح txt",
            data=valid_text,
...
if st.session_state.is_running and total_found < gen_limit:
    ...
    for _ in range(20): # فحص دفعات صغيرة لتحديث الواجهة
        needed = max(0, u_len - len(u_prefix))
        body = "".join(random.choices(chars + ("_" if use_under else ""), k=needed))
        user = u_prefix + body
        
        # شرط الشرطة (ليست في الأطراف)
        if use_under and len(user) >= 3:
            u_l = list(user)
            if u_l[0] == "_": u_l[0] = random.choice(chars)
            if u_l[-1] == "_": u_l[-1] = random.choice(chars)
            user = "".join(u_l)
